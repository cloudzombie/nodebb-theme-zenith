...
$(document).ready(function() {
	...
	function setupTypeIt() {
		$(window).on('action:ajaxify.end', function() { 
			$('.js-notloggedin-is').typeIt();
			$('.js-btn-notloggedin-remove').click(function() {
				$("html").removeClass("is-hidden");
				$('.content-notloggedin-cover').remove();
			});	
		});
	}
	function setupDailyMessages() {
		$(window).on('action:ajaxify.end', function() { 
			$('.js--footer-daily').dailyMessages({
			    monday: "Here we go, another Monday.",
			    tuesday: "Convoe goes up on a Tuesday.",
			    wednesday: "Keep calm, its already Wednesday.",
			    thursday: "We all know Thursday is the new Friday.",
			    friday: "Yes! We made it to another Friday!",
			    saturday: "Aye, its Saturday. Get off the Internet and go outside.",
			    sunday: "Its Sunday so... maybe some Netflix and chill? or nah.",
			    success: function() {
			        //console.log('');
			    },
